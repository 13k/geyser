...
func NewAPIGen(interfaces geyser.SchemaInterfacesGroup, pkgName, outputDir, baseFilename string) (*APIGen, error) {
...
func (g *APIGen) genStructCtor() j.Code {
	...
	getArgs := []j.Code{g.jenSchemaInterfaceKey(g.baseName, appID)}
...
func (g *APIGen) genMethod(methodName string, group geyser.SchemaMethodsGroup) j.Code {
	...
	for _, version := range versions {
		...
		for _, param := range sm.Params {
			comments = append(comments, paramComment(param))
		}
	}
	...
	getArgs := []j.Code{g.jenSchemaMethodKey(methodName, version)}
...
func (g *APIGen) genMethods() j.Code {
	...
	for _, name := range g.methodsNames {
		method := g.genMethod(name, g.groupedMethods[name])
