...
API interfaces and methods are generated automatically by sub-package "apigen" and are structured as
a tree.

NOTE: A non-partner api key is used to fetch the API schema, so only non-partner interfaces
are generated.
  ...
  |-> Interface([appID])
      |-> Method([version])
          |-> Request
              ...
              |-> Request execution
              |   |-> HTTP response
              |       |-> Check HTTP status and parse response body
              |-------|-> Check HTTP status and use the configured result value
...
Results of requests can be obtained in two ways: parsing the response body manually, or configuring
the result object in the Request before executing.
...
the result object in the Request (see Request.SetResult), a "JSON-unmarshable" object is expected.
...
interface struct name and the method name (e.g.: "ISteamApps/GetAppList" -> "SteamAppsGetAppList").

Given that each response has a specific format and cannot be automatically generated, most of these
result structs are empty (so they are useless, but response bodies are still available for manual
parsing). When a response format is known, the result struct will be updated and won't be
automatically (re)generated.

Look for non-empty result structs for each interface method to see if there's any available at the
time. Contributions to implement proper result structs are welcome!
...
API schema is defined by "Schema*" types that compose the tree-like structure of the generated API
and provide basic methods for navigating and manipulation of the schema.

A schema starts with a root SchemaInterfaces that is a collection of SchemaInterface interfaces. It
also provides helper methods to group interfaces by name, extract AppIDs and so on.

SchemaInterface holds the specificiation of the interface and a SchemaMethods collection of methods.

SchemaMethods is a collection of SchemaMethod and provides helpers methods to group methods by name,
extract versions and so on.

SchemaMethod represents the specification of an interface method. It also contains a collection of
parameters SchemaMethodParams.
...
All of the collection types provide JSON encoding methods that help in serialization/deserialization
to/from JSON format. These types can be used directly when deserializing a JSON schema.
